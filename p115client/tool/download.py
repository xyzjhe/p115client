        ...
        def request(attrs: Sequence[dict], /):
            ...
            if "sha1" in attrs[0]:
                if get_raw:
                    ...
                    for attr in attrs:
                        if items := nodes.get(attr["pickcode"]):
                            attr["name"] = name
                return attrs
            else:
                if get_raw:
                    nodes = {
                        node["pick_code"]: (
                            ("fn", unescape_115_charref(node["file_name"])), 
                            ("sha1", node["sha1"]), 
                        ) for node in resp["data"]
                    }
                    for attr in attrs:
                        if items := nodes.get(attr["pc"]):
                            attr.update(items)
                else:
                    nodes = {
                        node["pick_code"]: (
    ...
    if path_already:
        def iter_nonroot(pickcode: str, /):
            return do_map(update_attr, iter_download_nodes(
                ...
                max_workers=max_workers, 
                max_page=max_files and -(-max_files // 5000), 
                app=app, 
                async_=async_, 
                **request_kwargs, 
            ))
        ...
        def gen_step(pickcode: str, /):
            ...
            def load_ancestors(pickcode: str, /):
                return through(iter_download_nodes(
                    ...
                    id_to_dirnode=id_to_dirnode, 
                    max_workers=max_workers, 
                    max_page=max_dirs and -(-max_dirs // 5000), 
                    app=app, 
                    async_=async_, 
                    **request_kwargs, 
                ))
